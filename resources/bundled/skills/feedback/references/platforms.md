...
```bash
python3 <skill_root>/scripts/resolve_platform.py
```

Where `<skill_root>` is the trusted bundled feedback-skill directory that contains this file (`<skill_root>/references/platforms.md`). Never execute `scripts/...` relative to the current working directory.
...
```bash
python3 <skill_root>/scripts/resolve_warp_version.py
```
